...
| **飞书 Wiki / 文档** `*.feishu.cn` `*.larksuite.com` | 标题、正文、标题层级、引用、列表、**表格**、图片、链接 | 自动滚动加载后按 block 提取；画板也能导出 |
...
## 和 CLI 版（web-clip）的区别：什么时候用哪个

同名能力有两种形态，互补：

- **用本插件**：你正在浏览器里看到一篇想**随手存**，点一下即可；尤其**需要登录才能看的页面**（知乎 / 飞书 / 公司内网）直接用你当前会话，无需额外配认证；加载即用、零环境依赖。
- **用 [CLI skill](https://github.com/1145434396-boop/web-clip)**：让 Claude / agent **自动、批量**剪藏、落到**任意指定目录**，或要抓**飞书文档内嵌的电子表格 / 多维表格**数据——这些是命令行版才有的。
